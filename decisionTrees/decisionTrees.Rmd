...
Con esto terminaríamos la imputación de los datos a priori, a la hora de decidir con cual nos quedaremos de las 3 imputaciones, en cuanto a resultados podríamos quedarnos con Amelia por rendimiento.
...
Para probar otras alternativas hemos pensado en utilizar otros dos paquetes de la misma librería, en este caso Ensemble Filter (EF). Este método divide el dataset en nparticiones y se aplican diferentes algoritmos de aprendizaje (C4.5, 1-KNN y LDA) sobre cada combinación de las particiones disponibles menos 1 y despues es probado con la que hemos dejado fuera, la restante. Finalmente, se aplica un esquema de voto por consenso o mayoría para eliminar las instancias ruidosas, de modo que si es por consenso se elimina si todos los clasificadores dicen que está mal clasificada o si es por mayoría se elimina si al menos la mitad de los clasificadores dicen que está mal clasificada.
...
La otra alternativa es Cross-Validated Committees Filter (CVCF), la cual es similar a EF pero tiene dos principales diferencias:
  - El único algoritmo de aprendizaje que se utiliza es C4.5 ya que los algoritmos de árboles son los que mejor trabajan con datasets ruidosos.
  - Cada clasificador aprendido mediante cross validation es usado para etiquetar todas las instancias del training, no solo las del test, como correctas o mal clasificadas.
...
Synthetic Minority Over-sampling Technique (SMOTE) es un algoritmo que nos permite hacer oversampling y undersampling al mismo tiempo, sin embargo lo hemos aplicado solo para oversamping. Básicamente escogemos un ejemplo de la clase minoritaria y se seleccionan al azar un número de vecinos más cercanos de la misma clase, no más de 5. Después se generan nuevos datos entre el punto de la clase minoritaria seleccionada y sus vecinos más cercanos elegidos.
...
Random Oversampling (ROS) básicamente consiste en generar muestras aleatorias de la clase minoritaria.
...
Básicamente consiste en hacer parejas de instancias mixtas, positivas y negativas, que estén muy cercanas entre sí y eliminarlas. Esto nos hace reducir ruido e instancias de las clases mayoritarias en las fronteras.
```{r tomeklink}
set.seed(2)
...
Otra gran ventaja de la selección de características es que simplifican mucho el coste computacional de la clasificación, ya que reducimos la dimensionalidad del conjunto de datos y por tanto ahorramos tiempo de cómputo.

Por último, cabe mencionar que este proceso mejora enormemente la interpretabilidad del problema ya que nos resume el modelo en variables que son muy importantes a la hora de aprender el clasificador y que por tanto tenemos que tener en cuenta y nos ayuda a descartar aquellas que son inútiles o aportan poca información de utilidad.
...
```{r }
print(modelLMT)
pres <- as.party(modelLMT)
partykit:::.list.rules.party(pres)

proporcion = group_by(datosSmote,C) %>% summarise(propo = (nc = n()))
proporcion
...
Vamos a ordenar las instancias por puntuación pública, después también comentaremos su puntuación privada:

1. Quitando correlaciones con la variable de clase ... etc - 1 en kaggle public: 0.90964 pero en privado 0.90102. Este modelo lo aprendimos usando amelia, ipf para ruido, undersampling, correlacion, LMT y haciendo selección de variables mediante correlación con la variable de clase escogiendo las 25 mejores.

2. LMT y descartando variables por correlaciones - 2 en kaggle public 0.90301 pero en privado 0.90000.


3. LMT con todas las variables - 3 en kaggle public igual que el anterior pero en privado el mejor de todos: 0.90561.

4. LMT quitando variables con random forest - 4: 0.90505 y en privado 0.90204

5. quitando correlaciones con la variable de clase - 5
private 0.90051
public 0.90096
...
8. El mejor de J48 - 8: en privado 0.89030, en público 0.89127

9. en private0.90459 en public 0.90045

10. En private: 0.90306 en public0.90199