...
```{r}
datosPrep <- caret::preProcess(datos.train[,-ncol(datos.train)],method = c("center","scale"))
datos[,-ncol(datos.train)] <- predict(datosPrep,datos[,-ncol(datos)])
datosPrep <- caret::preProcess(test,method = c("center","scale"))
test <- predict(datosPrep,test)
```
...
datos.train$C <- as.numeric(datos.train$C)
```
...
datos.train.smote$C <- as.numeric(datos.train.smote$C)
```
