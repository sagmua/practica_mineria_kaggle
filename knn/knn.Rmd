...
EL MODELO OBTENIDO NOS DA UN ACCURACY EN TRAIN DE 0.9995869 Y EN TEST DE 0.79989 POR LO QUE PODEMOS DECIR QUE EL MODELO SOBREAJUSTA. -> NOTA: Habría que ver si hay que eliminar menos variables o más.
...
out <- IPF(C~., data = datos.train ,s = 3)
summary(out, explicit =TRUE)
out$cleanData
datosSinRuido <- datos.train[setdiff(1:nrow(datos.train),out$remIdx),]
cat("Se han eliminado ", dim(datos.train)[1]- dim(datosSinRuido)[1], "instancias")
datos.train <- datosSinRuido
```

```{r}
m <- cor(datos.train[,-ncol(datos.train)])
correlados <- findCorrelation(m, cutoff = 0.8)
datos.train <- datos.train[,-correlados]
datos.test <- datos.test[,-correlados]
...
```{r}
datos.train.smote<- oversample(datos.train,ratio=0.65, method="SMOTE",classAttr = "C")
table(datos.train.smote$C)
```
...
```{r,echo=FALSE}
set.seed (1)
datos.train.smote$C <- as.factor(datos.train.smote$C)
out <- IPF(C~., data = datos.train.smote ,s = 3)
summary(out, explicit =TRUE)
out$cleanData
datosSinRuido <- datos.train.smote[setdiff(1:nrow(datos.train.smote),out$remIdx),]
cat("Se han eliminado ", dim(datos.train.smote)[1]- dim(datosSinRuido)[1], "instancias")
datos.train.smote <- datosSinRuido
```
...
```{r}
resOutliers <- mvoutlier::uni.plot(datos.train.smote[,-ncol(datos.train.smote)])
```


```{r}
dim(datos.train.smote)
datos.train.smote.sinoutliers <- datos.train.smote[!resOutliers$outliers, ]
dim(datos.train.smote.sinoutliers)
```
...
```{r}
train_pred <- generarModelo(train = datos.train.smote.sinoutliers, test = datos.test[,-ncol(datos.test)],k=3)
...
```{r}
test_pred <- generarModelo(train = datos.train.smote.sinoutliers, test = test,3)
```
