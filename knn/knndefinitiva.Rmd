...
#Envio solo con cuartil inferior
```{r}
datosOutliersNA.imputadosPMM $C <- as.factor(datosOutliersNA.imputadosPMM $C)
...
#Envio con cuartil inferior y suoperior y eliminacion de ruido

```{r}
datosPrep <- caret::preProcess(datosOutliersNA.imputadosPMM.IPF[,-ncol(datosOutliersNA.imputadosPMM.IPF)],method = c("center","scale"))
datosOutliersNA.imputadosPMM.IPF[,-ncol(datosOutliersNA.imputadosPMM.IPF)] <- predict(datosPrep,datosOutliersNA.imputadosPMM.IPF[,-ncol(datosOutliersNA.imputadosPMM.IPF)])
```

```{r}
boxplot(datosOutliersNA.imputadosPMM.IPF)
```

#Modelo 7

```{r}
datosOutliersNA.imputadosPMM.IPF$C <- as.factor(datosOutliersNA.imputadosPMM.IPF$C)
model <- generarModelo(datosOutliersNA.imputadosPMM.IPF)
model
```
...
```{r}
generarEnvio(y=test_pred,file="envioDefinitivo7.csv")
```
